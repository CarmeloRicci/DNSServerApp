...
export default class DnsService {
  ...
  async InsertRecordHostsFile(device: IHostDevice) {
    ...
    const { stdout, stderr } = await exec(` sudo /bin/sh -c 'export TEST=$(echo $(ps axf | grep dnsmasq-2.8 | grep -v grep | awk '{print $1}'))'`);
    console.log('RESOLV: stdout:', stdout);
    console.log('RESOLV: stderr:', stderr);
    const temp = await delay(1000);
    const { stdout1, stderr2 } = await exec(` sudo /bin/sh -c 'kill -SIGHUP $TEST'`);
